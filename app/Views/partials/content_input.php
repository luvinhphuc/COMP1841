...
$contentInputType = trim((string) ($contentInputType ?? 'post'));
$isCommentInput = $contentInputType === 'comment';
$contentInputValue = (string) ($contentInputValue ?? '');
$contentInputErrors = is_array($contentInputErrors ?? null) ? $contentInputErrors : [];

$inputId = $isCommentInput ? 'comment-create' : 'post-create';
$contentId = $isCommentInput ? 'reply-content' : 'content';
$contentLabel = $isCommentInput ? 'Comment content' : 'Description';
$contentError = trim((string) ($contentInputErrors['content'] ?? ''));
$attachmentError = trim((string) ($contentInputErrors['attachment'] ?? ''));
$placeholder = $isCommentInput ? 'Share a clear explanation, useful resource, or next step.' : 'Body text';
$rows = $isCommentInput ? 6 : 7;
$maxLength = $isCommentInput ? 5000 : null;
$required = $isCommentInput;
$showAttachment = $isCommentInput || !empty($showAttachmentField);
$allowVideo = !$isCommentInput;
$attachmentAccept = $isCommentInput
...
$attachmentHelp = $isCommentInput ? 'Drag and drop or upload an image' : 'Drag and drop or upload media/files';
$submitLabel = $isCommentInput ? 'Comment' : '';

$attachmentId = $inputId . '-attachment';
$contentErrorId = $contentId . '-error';
$attachmentErrorId = $attachmentId . '-error';
$helpId = $attachmentId . '-help';
$borderClass = $contentError !== ''
...
<div data-content-input data-default-accept="<?= htmlspecialchars($attachmentAccept, ENT_QUOTES, 'UTF-8') ?>">
    <label for="<?= htmlspecialchars($contentId, ENT_QUOTES, 'UTF-8') ?>" class="sr-only">
        <?= htmlspecialchars($contentLabel, ENT_QUOTES, 'UTF-8') ?>
    </label>
    <div
        class="relative overflow-hidden rounded-[20px] border <?= $borderClass ?> bg-white transition focus-within:ring-2"
    ...
    >
        ...
        <textarea
            id="<?= htmlspecialchars($contentId, ENT_QUOTES, 'UTF-8') ?>"
            name="content"
            rows="<?= htmlspecialchars($rows, ENT_QUOTES, 'UTF-8') ?>"
            <?= $maxLength !== null ? 'maxlength="' . htmlspecialchars($maxLength, ENT_QUOTES, 'UTF-8') . '"' : '' ?>
            <?= $required ? 'required' : '' ?>
            aria-describedby="<?= htmlspecialchars($contentErrorId, ENT_QUOTES, 'UTF-8') ?>"
            ...
            data-content-field
        ><?= htmlspecialchars($contentInputValue, ENT_QUOTES, 'UTF-8') ?></textarea>

        <?php if ($showAttachment): ?>
        <input
            id="<?= htmlspecialchars($attachmentId, ENT_QUOTES, 'UTF-8') ?>"
            name="attachment"
            type="file"
            accept="<?= htmlspecialchars($attachmentAccept, ENT_QUOTES, 'UTF-8') ?>"
            aria-describedby="<?= htmlspecialchars($helpId . ' ' . $attachmentErrorId, ENT_QUOTES, 'UTF-8') ?>"
            aria-invalid="<?= $attachmentError !== '' ? 'true' : 'false' ?>"
            class="sr-only"
            data-attachment-input
        >
        <label
            for="<?= htmlspecialchars($attachmentId, ENT_QUOTES, 'UTF-8') ?>"
            class="absolute inset-0 z-10 hidden cursor-pointer flex-col items-center justify-center gap-3 rounded-[20px] border border-dashed <?= $attachmentError !== '' ? 'border-[#ba1a1a]' : 'border-[#c4c7c7]' ?> bg-white px-5 py-8 text-center transition hover:border-[#315f90] hover:bg-[#f7f9fd]"
        ...
        >
            ...
            <span id="<?= htmlspecialchars($helpId, ENT_QUOTES, 'UTF-8') ?>" class="text-sm leading-5 text-[#27313a]">
                <?= htmlspecialchars($attachmentHelp, ENT_QUOTES, 'UTF-8') ?>
            </span>
        </label>
    ...
    <p
        id="<?= htmlspecialchars($contentErrorId, ENT_QUOTES, 'UTF-8') ?>"
        class="mt-2 <?= $contentError === '' ? 'hidden' : 'block' ?> text-sm leading-5 text-[#ba1a1a]"
        aria-live="polite"
    >
        <?= htmlspecialchars($contentError, ENT_QUOTES, 'UTF-8') ?>
    </p>
    <?php if ($showAttachment): ?>
    <p
        id="<?= htmlspecialchars($attachmentErrorId, ENT_QUOTES, 'UTF-8') ?>"
        class="mt-2 <?= $attachmentError === '' ? 'hidden' : 'block' ?> text-sm leading-5 text-[#ba1a1a]"
        aria-live="polite"
    >
        <?= htmlspecialchars($attachmentError, ENT_QUOTES, 'UTF-8') ?>
    </p>
    <?php endif; ?>
</div>
